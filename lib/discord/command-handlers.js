...
const PLAYER_HELP_LINES = [
  ...
  '/list <flower> [extra_points] - Log a flower you own.',
  '/rem <flower> - Remove one logged flower and its pin.',
  '/listall <flower_pattern> - Pick matching flowers to log.',
  ...
  '/setname <name> - Set your Cosy Florist game name.',
...
const ADMIN_HELP_LINES = [
  '/addflower - Add or update a flower in the database.',
...
async function handleSetLevel(interaction, sql) {
  ...
  if (!Number.isInteger(level) || level < 0) {
    return ephemeralMessage('Level must be 0 or higher.');
  }
  ...
  return ephemeralMessage(`Logged ${result.count} assignment-level flower${result.count === 1 ? '' : 's'} up to level ${level}.`);
}
...
async function handleFindPoints(interaction, sql) {
  ...
  if (!Number.isInteger(points) || points < 0) {
    return ephemeralMessage('Points must be 0 or higher.');
  }
  ...
  return ephemeralMessage(
    truncate(
      [`${flowers.length} flower${flowers.length === 1 ? '' : 's'} have ${points} quest points.`, lines.join('\n')].filter(Boolean).join('\n'),
    ),
  );
}
...
async function handlePoints(interaction, sql) {
  const flower = await findFlowerByInput(sql, optionString(interaction, 'flower'));
  return flower
    ? ephemeralMessage(`${flower.name} has ${flower.quest_points} quest points.`)
    : ephemeralMessage('I could not find that flower.');
}
...
async function handlePin(interaction, sql) {
  ...
  if (!logged) {
    return ephemeralMessage(`You need to log ${flower.name} before you can pin it.`);
  }
  ...
  if (removed.count > 0) {
    return ephemeralMessage(`Unpinned ${flower.name}.`);
  }
  ...
  return ephemeralMessage(`Pinned ${flower.name}.`);
}
...
async function handlePinned(interaction, sql) {
  ...
  return ephemeralMessage(truncate([`${heading} (${rows.length}):`, lines.join('\n') || 'No pinned flowers.'].join('\n')));
}
...
async function handleAddFlower(interaction, sql) {
  ...
  return ephemeralMessage(`Saved ${name} (${rarity}, ${questPoints} pts).`);
}
...
async function createAdminRequest(interaction, sql, action) {
  const requesterId = assertUser(interaction);
...
async function handleAdminApprovalComponent(interaction, sql, action, requestId) {
  const actorId = assertUser(interaction);
...
export async function handleApplicationCommand(interaction, dependencies = {}) {
  ...
  switch (name) {
    ...
    case 'addadmin':
      return createAdminRequest(interaction, sql, 'add_admin');
    case 'removeadmin':
      return createAdminRequest(interaction, sql, 'remove_admin');
...
export async function handleMessageComponent(interaction, dependencies = {}) {
  ...
  if (kind === 'admin-approve') {
    return handleAdminApprovalComponent(interaction, sql, 'approve', first);
  }

  if (kind === 'admin-reject') {
    return handleAdminApprovalComponent(interaction, sql, 'reject', first);
  }
