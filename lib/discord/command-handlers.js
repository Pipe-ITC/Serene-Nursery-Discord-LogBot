...
function privateEmbedMessage(content, embeds) {
...
function ownedFlowerEmbeds(rows) {
...
async function handleOwnedBy(interaction, sql) {
  ...
  if (rows.length === 0) {
    return ephemeralMessage(`${content}\nNo logged flowers found.`);
  }

  return privateEmbedMessage(content, ownedFlowerEmbeds(rows));
}
