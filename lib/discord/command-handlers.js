...
import { markWeeklyDone, resetWeeklyDoneUsers } from '../weekly-done.js';
...
import { rarityDisplay, rarityEmoji } from './rarity-emojis.js';
import { ephemeralData, ephemeralMessage, message, updateMessage } from './responses.js';

const MAX_SELECT_OPTIONS = 25;
const MAX_SELECT_MENUS = 5;
const MAX_PICKER_OPTIONS = MAX_SELECT_OPTIONS * MAX_SELECT_MENUS;
...
const DEFAULT_FLASHING_STAR_EMOJI_ID = '1526187016717471825';
const DEFAULT_FLASHING_EXCLAMATION_EMOJI_ID = '1526190062105264259';
const DONE_TICK_EMOJI = '☑️';
const VALID_EXTRA_POINTS = new Set([1, 2, 3, 4]);
...
const PLAYER_HELP_LINES = [
  ...
  '/addpoints <flower> <points> - Set extra points for a logged flower.',
  '/del <flower> - Remove one logged flower and its pin.',
  '/logall <flower_pattern|rarity> - Pick matching flowers to log.',
  ...
  '/ownedby [player] [rarity] - Privately show logged flowers grouped by rarity.',
  '/cozyplayers - Show Cozy players and their pinned flowers.',
  '/done - Mark your weekly quests as done.',
];
const ADMIN_HELP_LINES = [
  () => `Admin dashboard: ${adminDashboardUrl()}`,
  '/addflower - Add a new flower to the database.',
  '/setflowerimage <flower> <image> - Upload or replace a flower image.',
  ...
  '/removeuser <user> - Remove a player record, logged flowers, and pins.',
  '/donereset - Clear all weekly quest done markers.',
...
function appEnvironment() {
  return (process.env.APP_ENVIRONMENT ?? 'development').trim().toLowerCase();
}

function adminDashboardUrl() {
  const environment = appEnvironment();
  if (environment === 'production') {
    return 'https://admin.serenenursery.pipeitc.net';
  }
  if (['development', 'dev', 'preview'].includes(environment)) {
    return 'https://admin.serenenursery.pipeitc.dev';
  }

  return (process.env.ADMIN_BASE_URL ?? 'https://admin.serenenursery.pipeitc.dev').replace(/\/$/, '');
}
...
function flashingStarEmoji() {
  return animatedEmoji('flashing_stars', process.env.EMOJI_FLASHING_STAR ?? DEFAULT_FLASHING_STAR_EMOJI_ID);
}

function flashingExclamationEmoji() {
  return animatedEmoji('flashingexclamationemoji', process.env.EMOJI_FLASHING_EXCLAMATION ?? DEFAULT_FLASHING_EXCLAMATION_EMOJI_ID);
}

function flowerLabel(flower) {
  const extraPoints = Number(flower.extra_points ?? 0);
  const extraLabel = extraPoints > 0 ? ` (+${extraPoints})` : '';
  return `${flower.name} (${rarityEmoji(flower.rarity)}, ${totalPoints(flower)} pts${extraLabel})`;
}

function rarityLabel(rarity) {
  return rarityDisplay(rarity);
}

function resolvedDiscordName(interaction, discordUserId) {
  const user = interaction?.data?.resolved?.users?.[discordUserId];
  return user?.global_name ?? user?.username;
}

function plainUserLabel(interaction, discordUserId, user = {}) {
  const gameName = user.game_name?.trim();
  const discordName = resolvedDiscordName(interaction, discordUserId);

  if (gameName && discordName) {
    return `${gameName} (${discordName})`;
  }

  return gameName || discordName || 'Selected user';
}
...
function privateEmbedMessage(content, embeds, components = []) {
...
function optionAttachment(interaction, name) {
  const attachmentId = optionValue(interaction?.data?.options, name);
...
function hasOption(interaction, name) {
  return optionValue(interaction?.data?.options, name) !== undefined;
}

function isValidExtraPoints(points) {
  return Number.isInteger(points) && VALID_EXTRA_POINTS.has(points);
}
...
async function matchingFlowers(sql, value, limit = MAX_PICKER_OPTIONS) {
  const [byId] = await sql`
...
async function matchingFlowersByRarity(sql, rarity, limit = MAX_PICKER_OPTIONS) {
  return sql`
    select id, name, rarity, quest_points
    from flowers
    where rarity = ${rarity}
...
function rarityFromExactInput(value) {
  const normalized = String(value ?? '').trim().toUpperCase();
  return RARITY_ORDER.includes(normalized) ? normalized : undefined;
}
...
function selectMenu(customId, flowers, placeholder) {
  return chunkLinesForSelectMenus(flowers).map((flowerChunk, index, chunks) => ({
    type: ComponentType.ACTION_ROW,
    components: [
      {
        type: ComponentType.STRING_SELECT,
        custom_id: chunks.length === 1 ? customId : `${customId}:${index + 1}`,
        placeholder: chunks.length === 1 ? placeholder : `${placeholder} (${index + 1}/${chunks.length})`,
        min_values: 1,
        max_values: Math.min(flowerChunk.length, MAX_SELECT_OPTIONS),
        options: flowerChunk.map((flower) => ({
...
function chunkLinesForSelectMenus(flowers) {
  const chunks = [];

  for (let index = 0; index < flowers.length; index += MAX_SELECT_OPTIONS) {
    chunks.push(flowers.slice(index, index + MAX_SELECT_OPTIONS));
  }
...
async function logAssignmentLevelFlowers(sql, discordUserId, level) {
  await ensureUser(sql, discordUserId);
  const result = await sql`
    insert into flower_logs (discord_user_id, flower_id, extra_points, logged_at)
    ...
    on conflict (discord_user_id, flower_id) do nothing
    returning id
  `;
...
function levelFanfareMessage(discordUserId, level, loggedCount) {
  return message('FANFARE!', {
    embeds: [
      {
        image: {
      ...
      },
      {
        description: [
          ...
          `The nursery gates swing wide: ${loggedCount} assignment-level flower${loggedCount === 1 ? '' : 's'} logged in one glorious burst.`,
          '',
...
function isAssignmentLevelFlower(flower) {
  return String(flower.rarity ?? '').toUpperCase() === 'N' && flower.assignment_level !== null && flower.assignment_level !== undefined;
}
...
async function handleList(interaction, sql) {
  ...
  if (hasOption(interaction, 'extra_points') && !isValidExtraPoints(extraPoints)) {
    return ephemeralMessage('Extra points must be 1, 2, 3, or 4.');
  }

  if (isAssignmentLevelFlower(flower)) {
    const level = Number(flower.assignment_level);
    const loggedCount = await logAssignmentLevelFlowers(sql, discordUserId, level);

    if (loggedCount === 0) {
      return ephemeralMessage(`You already have all assignment-level flowers up to level ${level}.`);
    }

    return levelFanfareMessage(discordUserId, level, loggedCount);
  }
  ...
  if (existingLog) {
    return ephemeralMessage(`You have already logged ${flower.name}. Use /addpoints if you need to update its extra points.`);
  }

  const [flowerLogStats] = await sql`
    select count(*)::int as count
    from flower_logs
    where flower_id = ${flower.id}
  `;
  const isFirstGuildLog = Number(flowerLogStats?.count ?? 0) === 0;

  await upsertLogs(sql, discordUserId, [flower.id], extraPoints);

  if (isFirstGuildLog) {
    const stars = flashingStarEmoji();
    return message([
      `${stars} FIRST BLOOM! ${stars}`,
      '',
      `<@${discordUserId}> is the first player to log ${flower.name}.`,
      '',
      'The nursery catalogue grows by one beautiful discovery.',
      'A fine moment for the garden, and a finer one for the florist.',
    ].join('\n'));
  }
...
async function handleAddPoints(interaction, sql) {
  const discordUserId = assertUser(interaction);
  const flower = await findFlowerByInput(sql, optionString(interaction, 'flower'));
  ...
  if (!isValidExtraPoints(points)) {
    return ephemeralMessage('Extra points must be 1, 2, 3, or 4.');
  }

  const [updated] = await sql`
    update flower_logs
    set extra_points = ${points}
    where discord_user_id = ${discordUserId}
      and flower_id = ${flower.id}
    returning id
  `;
  ...
  if (!updated) {
    return ephemeralMessage(`You need to log ${flower.name} before you can add extra points.`);
  }

  return ephemeralMessage(`Updated ${flowerLabel(flower)} to +${points} extra point${points === 1 ? '' : 's'}. Total for you: ${Number(flower.quest_points) + points}.`);
}
...
async function handleHelp(interaction, sql) {
  ...
  if (isAdmin) {
    sections.push('', 'Admin commands:', ADMIN_HELP_LINES.map((line) => `- ${helpLine(line)}`).join('\n'));
  }
...
async function handleListAll(interaction, sql) {
  const discordUserId = assertUser(interaction);
  const pattern = optionString(interaction, 'flower_pattern')?.trim();
  const selectedRarity = optionString(interaction, 'rarity');
  const patternRarity = rarityFromExactInput(pattern);

  if (pattern && selectedRarity) {
    return ephemeralMessage('Please choose either a flower pattern or a rarity, not both.');
  }

  if (!pattern && !selectedRarity) {
    return ephemeralMessage('Please enter a flower pattern or choose a rarity.');
  }

  const rarity = selectedRarity ?? patternRarity;
  const isRaritySearch = Boolean(rarity);
  const flowers = isRaritySearch ? await matchingFlowersByRarity(sql, rarity) : await matchingFlowers(sql, pattern);
  const matchLabel = isRaritySearch ? `${rarity} rarity` : pattern;

  if (flowers.length === 0) {
    return ephemeralMessage(`No flowers matched "${matchLabel}".`);
  }

  return ephemeralData({
    content: `Select the flowers to log from ${flowers.length} match${flowers.length === 1 ? '' : 'es'} for "${matchLabel}".`,
...
async function handleFind(interaction, sql) {
  ...
  const pinners = await sql`
    select u.discord_user_id, u.game_name, (d.discord_user_id is not null) as is_done
    from flower_pins p
    join app_users u on u.discord_user_id = p.discord_user_id
    left join weekly_done_users d on d.discord_user_id = p.discord_user_id
  ...
  for (const owner of owners) {
    players.set(owner.discord_user_id, {
      ...owner,
      isLogged: true,
      isPinned: false,
      extraPoints: Number(owner.extra_points ?? 0),
    });
  }
  for (const pinner of pinners) {
    const existing = players.get(pinner.discord_user_id);
    players.set(pinner.discord_user_id, {
      ...existing,
      ...pinner,
      isLogged: Boolean(existing?.isLogged),
      isPinned: true,
      isDone: Boolean(pinner.is_done),
      extraPoints: Number(existing?.extraPoints ?? 0),
    });
  }

  const playerLines = [...players.values()]
    .sort((left, right) => {
      const leftRank = playerFindRank(left);
      const rightRank = playerFindRank(right);
      if (leftRank !== rightRank) {
        return leftRank - rightRank;
      }

      return displayNameForUser(left.discord_user_id, left).localeCompare(displayNameForUser(right.discord_user_id, right));
    })
    .map((player) => {
      const status = playerFindStatus(player);
      const extra = player.extraPoints > 0 ? ` +${player.extraPoints}` : '';
      return `${status} ${displayNameForUser(player.discord_user_id, player)}${extra}`;
    });

  return message(
    truncate(
      [
        `${rarityEmoji(flower.rarity)} ${flower.name}`,
        `Quest points: ${flower.quest_points}`,
        '',
        `Players (${players.size}):`,
        playerLines.length ? playerLines.join('\n') : 'No one has logged or pinned this flower yet.',
      ].join('\n'),
    ),
  );
}

function playerFindRank(player) {
  if (player.isPinned && player.extraPoints > 0) {
    return 0;
  }
...
function playerFindStatus(player) {
  if (player.isPinned && player.isDone) {
    return DONE_TICK_EMOJI;
  }

  if (player.extraPoints > 0) {
    return flashingExclamationEmoji();
  }
...
async function handleFindPoints(interaction, sql) {
  ...
  const lines = flowers.map((flower) => `- ${flower.name} (${rarityEmoji(flower.rarity)})`);
...
async function handleFindRarity(interaction, sql) {
  ...
  const content = `${flowers.length} ${rarityEmoji(rarity)} flower${flowers.length === 1 ? '' : 's'} found.`;

  if (lines.length === 0) {
    return message(content);
  }

  return publicLineListMessage(content, lines, `${rarityEmoji(rarity)} flowers`);
}
...
async function handleInfo(interaction, sql) {
  ...
  const [firstLogger] = await sql`
    ...
    limit 1
  `;
  const firstLoggerName = firstLogger ? displayNameForUser(firstLogger.discord_user_id, firstLogger) : 'None';
  const embed = {
    title: `${rarityEmoji(flower.rarity)} ${flower.name}`,
    color: RARITY_COLORS[flower.rarity],
    description: [
      `🎯 Quest Points: **${flower.quest_points}**`,
      stats.logged_by_you ? '✅ Flower is logged by you' : '☑️ Flower is not logged by you',
      `👥 Logged By: **${stats.logged_count} Florist${stats.logged_count === 1 ? '' : 's'}**`,
      `📌 Pinned By: **${stats.pinned_count} Florist${stats.pinned_count === 1 ? '' : 's'}**`,
      `🥇 First Logger: **${firstLoggerName}**`,
    ].join('\n'),
  };

  if (flower.image_url) {
    embed.thumbnail = { url: flower.image_url };
  }
...
async function handleCount(interaction, sql) {
  ...
  const lines = RARITY_ORDER.map((code) => {
    ...
    return `- ${rarityEmoji(code)}: ${row.logged}/${row.total}`;
  });
...
async function handlePinned(interaction, sql) {
  ...
  const [targetUser] = targetUserId === requesterId
    ? []
    : await sql`
        select game_name
        from app_users
      ...
      `;
  const rows = await sql`
    select f.name, f.rarity, f.quest_points, l.extra_points
    from flower_pins p
    join flowers f on f.id = p.flower_id
    join flower_logs l on l.discord_user_id = p.discord_user_id and l.flower_id = p.flower_id
    where p.discord_user_id = ${targetUserId}
    order by f.name
  `;
  const heading = targetUserId === requesterId ? 'Your pinned flowers' : `${plainUserLabel(interaction, targetUserId, targetUser)} pinned flowers`;
...
function ownedFlowerLine(flower) {
  const extraPoints = Number(flower.extra_points ?? 0);
  const extraLabel = extraPoints > 0 ? ` (+${extraPoints})` : '';
  return `- ${flower.name} (${totalPoints(flower)} pts${extraLabel})`;
}

function ownedFlowerPanels(rows) {
  const grouped = new Map(RARITY_ORDER.map((rarity) => [rarity, []]));
  ...
  for (const row of rows) {
    ...
    grouped.get(row.rarity).push(ownedFlowerLine(row));
  }

  const panels = [];
  for (const rarity of RARITY_ORDER) {
    const lines = grouped.get(rarity) ?? [];
    if (lines.length === 0) {
      continue;
    }

    const chunks = chunkLines(lines, MAX_EMBED_DESCRIPTION_LENGTH);
    for (const [index, description] of chunks.entries()) {
      panels.push({
        rarity,
        count: lines.length,
        title: index === 0 ? `${rarityEmoji(rarity)} logged flowers` : `${rarityEmoji(rarity)} logged flowers continued`,
...
function ownedByCustomId(requesterId, targetUserId, rarity, page) {
  return ['ownedby', requesterId, targetUserId, rarity ?? 'all', String(page)].join(':');
}

function ownedByPageComponents(requesterId, targetUserId, rarity, page, pageCount) {
  if (pageCount <= 1) {
    return [];
  }
      ...
      components: [
        {
          ...
          custom_id: ownedByCustomId(requesterId, targetUserId, rarity, Math.max(0, page - 1)),
          label: 'Previous',
          disabled: page <= 0,
        },
        {
          type: ComponentType.BUTTON,
          style: ButtonStyle.SECONDARY,
          custom_id: ownedByCustomId(requesterId, targetUserId, rarity, Math.min(pageCount - 1, page + 1)),
          label: 'Next',
...
function ownedByResponse({ requesterId, targetUserId, subject, rarity, rows, page = 0, isUpdate = false }) {
  const rarityText = rarity ? ` ${rarityEmoji(rarity)}` : '';
  const heading = `${subject} logged flowers`;
  const content = `${heading}${rarityText} (${rows.length}):`;

  if (rows.length === 0) {
    const payload = { content: `${content}\nNo logged flowers found.`, embeds: [], components: [] };
    return isUpdate ? updateMessage(payload) : ephemeralData(payload);
  }

  const panels = ownedFlowerPanels(rows);
  const clampedPage = Math.min(Math.max(page, 0), panels.length - 1);
  const panel = panels[clampedPage];
  const pageContent = panels.length > 1 ? `${content}\nPanel ${clampedPage + 1}/${panels.length}` : content;
  const payload = {
    content: pageContent,
    embeds: [
      {
    ...
    components: ownedByPageComponents(requesterId, targetUserId, rarity, clampedPage, panels.length),
  };

  return isUpdate ? updateMessage(payload) : privateEmbedMessage(payload.content, payload.embeds, payload.components);
}

async function ownedByRows(sql, targetUserId, rarity) {
  return rarity
    ? await sql`
        select f.name, f.rarity, f.quest_points, l.extra_points
        from flower_logs l
        join flowers f on f.id = l.flower_id
        where l.discord_user_id = ${targetUserId}
          and f.rarity = ${rarity}
        order by array_position(array['N','R','SR','SSR','UR'], f.rarity), f.name
      `
    : await sql`
        select f.name, f.rarity, f.quest_points, l.extra_points
        from flower_logs l
...
async function handleOwnedBy(interaction, sql) {
  const requesterId = assertUser(interaction);
  const targetUserId = optionString(interaction, 'player') ?? requesterId;
  const rarity = optionString(interaction, 'rarity');

  if (rarity && !RARITY_ORDER.includes(rarity)) {
    return ephemeralMessage('Please choose a valid rarity.');
  }
  ...
  const rows = await ownedByRows(sql, targetUserId, rarity);
  const subject = targetUserId === requesterId ? 'Your' : `${plainUserLabel(interaction, targetUserId, targetUser)}`;

  return ownedByResponse({ requesterId, targetUserId, subject, rarity, rows });
}

async function handleOwnedByComponent(interaction, sql, requesterId, targetUserId, rarityValue, pageValue) {
  const actorId = assertUser(interaction);
  if (actorId !== requesterId) {
    return ephemeralMessage('Only the user who ran /ownedby can use these buttons.');
  }

  const rarity = rarityValue === 'all' ? undefined : rarityValue;
  if (rarity && !RARITY_ORDER.includes(rarity)) {
    return ephemeralMessage('Please choose a valid rarity.');
  }
  ...
  const subject = targetUserId === requesterId ? 'Your' : displayNameForUser(targetUserId, targetUser);
...
async function handleCozyPlayers(interaction, sql) {
  assertUser(interaction);
  ...
  const rows = await sql`
    select
      ...
      f.quest_points
    from app_users u
    left join flower_pins p on p.discord_user_id = u.discord_user_id
    left join flowers f on f.id = p.flower_id
    where u.discord_user_id like 'cozy:%'
    order by lower(coalesce(nullif(u.game_name, ''), u.discord_user_id)), f.name
  `;
  const players = new Map();

  for (const row of rows) {
    if (!players.has(row.discord_user_id)) {
      players.set(row.discord_user_id, {
    ...
    if (row.name) {
      players.get(row.discord_user_id).flowers.push({
        name: row.name,
        rarity: row.rarity,
  ...
  if (players.size === 0) {
    return message('No Cozy players found.');
  }

  const lines = [];
  for (const player of players.values()) {
    lines.push(`**${displayNameForUser(player.discord_user_id, player)}**`);

    if (player.flowers.length === 0) {
      lines.push('No pinned flowers.');
      continue;
    }

    for (const flower of player.flowers) {
      lines.push(`📌 ${flowerLabel(flower)}`);
    }
  }

  return publicLineListMessage(`Cozy players (${players.size}):`, lines, 'Cozy players');
}
...
async function handleAddFlower(interaction, sql) {
  ...
  const imageAttachment = optionAttachment(interaction, 'image');
  const imageUrl = optionString(interaction, 'image_url')?.trim() || attachmentImageUrl(imageAttachment) || null;
  ...
  if (imageAttachment && !attachmentImageUrl(imageAttachment)) {
    return ephemeralMessage('Please upload an image file for the flower image.');
  }
  ...
  return message(`New flower added: ${name} (${rarityEmoji(rarity)}, ${questPoints} pts). It is now available to log.`);
}

async function handleSetFlowerImage(interaction, sql) {
  const discordUserId = assertUser(interaction);
  if (!(await assertAdmin(sql, discordUserId))) {
    return ephemeralMessage('Only app admins can use this command.');
  }
  ...
  const flower = await findFlowerByInput(sql, optionString(interaction, 'flower'));
  const imageUrl = attachmentImageUrl(optionAttachment(interaction, 'image'));
  ...
  return ephemeralMessage(`Updated the image for ${flower.name}.`);
}
...
async function handleDone(interaction, sql) {
  const discordUserId = assertUser(interaction);
  await markWeeklyDone(sql, discordUserId);

  return ephemeralMessage('👍 You have marked all your quests as done');
}

async function handleDoneReset(interaction, sql) {
  const requesterId = assertUser(interaction);
  if (!(await assertAdmin(sql, requesterId))) {
    return ephemeralMessage('Only app admins can use this command.');
  }

  const result = await resetWeeklyDoneUsers(sql);
  return ephemeralMessage(`Cleared ${result.count} weekly quest done marker${result.count === 1 ? '' : 's'}.`);
}

async function handleRemoveUser(interaction, sql) {
  const requesterId = assertUser(interaction);
  if (!(await assertAdmin(sql, requesterId))) {
    return ephemeralMessage('Only app admins can use this command.');
  }
  ...
  if (!user) {
    return ephemeralMessage(`No Serene Nursery user record exists for <@${targetUserId}>.`);
  }

  if (user.is_admin) {
    return ephemeralMessage('App admins cannot be removed with /removeuser. Use /removeadmin first.');
  }

  const [counts] = await sql`
    select
      (select count(*)::int from flower_logs where discord_user_id = ${targetUserId}) as logged_flowers,
      (select count(*)::int from flower_pins where discord_user_id = ${targetUserId}) as pins
  `;

  await sql`delete from app_users where discord_user_id = ${targetUserId}`;

  return ephemeralMessage(
    `Removed ${displayNameForUser(targetUserId, user)}. Deleted ${counts.logged_flowers} logged flower${counts.logged_flowers === 1 ? '' : 's'} and ${counts.pins} pin${counts.pins === 1 ? '' : 's'}.`,
  );
}
...
export async function handleApplicationCommand(interaction, dependencies = {}) {
  ...
  switch (name) {
    ...
    case 'addpoints':
      return handleAddPoints(interaction, sql);
    case 'del':
    ...
    case 'ownedby':
      return handleOwnedBy(interaction, sql);
    case 'cozyplayers':
      return handleCozyPlayers(interaction, sql);
    case 'done':
      return handleDone(interaction, sql);
    case 'addflower':
      return handleAddFlower(interaction, sql);
    case 'setflowerimage':
      return handleSetFlowerImage(interaction, sql);
    case 'pinned-flowers':
    ...
    case 'removeuser':
      return handleRemoveUser(interaction, sql);
    case 'donereset':
      return handleDoneReset(interaction, sql);
...
export async function handleMessageComponent(interaction, dependencies = {}) {
  ...
  const [kind, first, second, third, fourth] = customId.split(':');
  ...
  if (kind === 'ownedby') {
    return handleOwnedByComponent(interaction, sql, first, second, third, fourth);
  }
